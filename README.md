...
scripts/xctest-junit.py        # Runs XCTest and writes its results as JUnit XML (for Codecov)
codecov.yml                    # Codecov settings for CI's coverage and test result uploads
...
CI runs the suite in the build job (macOS and Mac Catalyst, without a dictionary) and again in the test job with the full V1 dictionary, pinned by version and SHA-256 in `build.yml`. The test job caches the dictionary zips between runs (`actions/cache`, one entry per zip, keyed on the version, that zip's SHA-256 pin and the hash of `scripts/fetch-test-dictionary.sh`), and `scripts/fetch-test-dictionary.sh` checks each zip's SHA-256 on every run, whether it came from the cache or was just downloaded. The release workflow also runs the suite, with the same dictionary, against the binary it is about to publish; it downloads the dictionary instead of using the cache (see the note in `release.yml`). The build and test jobs also write each run's results as JUnit XML (`scripts/xctest-junit.py`, since `swift test --xunit-output` covers XCTest only with `--parallel`), which CI uploads to Codecov Test Analytics: its Tests tab tracks each test's failure rate and duration and flags flaky tests, and when tests fail, Codecov lists them in its PR comment, on pull requests from forks too. Each test's name ends with its run, as in `testFoo (Mac Catalyst)`, because Codecov would otherwise count a test's three runs as one.

The Rust unit tests are in `rust/src/tests.rs`, not in `lib.rs`, so that coverage counts only the wrapper's own code: cargo-llvm-cov leaves files named `tests.rs` out of its report. CI runs them with [cargo-llvm-cov](https://github.com/taiki-e/cargo-llvm-cov), runs the test job's `swift test` with `--enable-code-coverage`, and uploads both reports to [Codecov](https://codecov.io/gh/h1431532403240/sudachi-swift) (flags `rust` and `swift`, settings in `codecov.yml`); each run's summary page shows the same tables. Codecov posts its coverage status (`codecov/patch`; the free Developer plan this repository is on has no `codecov/project`) and PR comment when the codecov job's last step reports the uploads done (`manual_trigger` in `codecov.yml`), or earlier by itself once the Swift test results show no failures; when Swift tests fail, the comment lists them instead and no coverage statuses are posted. On pull requests from forks, that last step has no OIDC token and is refused unless the Codecov account doesn't require upload tokens, so coverage statuses and comment come only from passing test results, and not reliably: Codecov skips them if a coverage upload is being processed at that moment. The reports cover `rust/src/lib.rs` and `Sources/SudachiSwift/SudachiSwift.swift`. The generated bindings are left out, and the Rust code that the Swift tests reach through the XCFramework isn't measured, because the XCFramework is a release build without coverage instrumentation. For the Rust report locally: `rustup component add llvm-tools-preview`, `cargo install cargo-llvm-cov --version 0.9.1 --locked`, then `cargo llvm-cov` in `rust/`.
